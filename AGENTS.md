...
- Single-page app: one `index.html` with a tab-style nav that swaps between four views (no page reloads): Agent Spark, Blueprint Lab, Admin ("สำหรับทีม Admin"), and Live (projection screen — opened from the Admin dashboard, not the public nav).
...
## Live / celebration features (2026-07)
- On successful Agent Spark submit, `sparkCelebrate()` replaces the form with a confetti + "คุณคือคนที่ N" screen (N from the `submission_count()` RPC) and an "ออกแบบอีกอัน" reset button. `confettiBurst()` (canvas, no library) and `showToast()` are shared helpers in app.js.
- **Live view** (`view-live`, admin-gated — the public nav has no Live button; it's opened via a "🔴 เปิดจอ Live" button inside the Admin dashboard, and `showView('live')` bails to Admin unless `isAdmin` is set after a successful `adminLoad`): a big auto-updating counter + Kahoot-style joiner nicknames for projecting during the workshop. Polls `live_stats(since)` every 4s (`liveTick`/`startLive`/`stopLive`, started/stopped by `showView`); on increase → confetti + a toast per new nickname. Polling (not Realtime) is used here on purpose so anon never needs row SELECT access. The nickname comes from Agent Spark's optional `display_name` field (reuses the existing `display_name` column; falls back to "ผู้ร่วมสนุก"). A **client-only Reset** button sets `liveSince = now` in `localStorage` so the on-screen count/names restart from 0 for a new talk round — it never deletes DB rows (Admin still shows the true total).
- **Admin Realtime**: `setupAdminRealtime()` subscribes to `postgres_changes` INSERT on `agent_blueprints` (admin is authenticated + in `workshop_admins`, so RLS allows it). On insert → toast + confetti + debounced `adminLoad()` reload.
- **Required SQL** (in supabase.sql, must be run on Supabase): `submission_count()` (total, for the Spark celebration "คุณคือคนที่ N") and `live_stats(since)` (count + recent nicknames since a timestamp, for the Live screen) — both SECURITY DEFINER, granted anon/authenticated, returning only counts/nicknames never full rows (safe under RLS); plus `agent_blueprints` added to the `supabase_realtime` publication (idempotent DO block) for admin toasts. Without these: Live counter/names stay empty and admin toasts never fire (app still works, degrades gracefully).
